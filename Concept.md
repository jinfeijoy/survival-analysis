...
* **Life expectancy**: life expectancy can be expressed as an integral of the survival curve
  * Probability of death at or before age `t0 + t` given survival until age `t0` is:<img src="https://render.githubusercontent.com/render/math?math=S(t)=P(T\le t_0 \bm{+} t|T>t_0)= \frac{P(t_0<T\le T_0+t)}{P(T>t_0)}=\frac{F(t_0+t)-F(t_0)}{S(t_0)}"> 
* **Censoring**:
